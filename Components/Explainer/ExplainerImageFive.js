...
class ExplainerImageFive extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
        //sessionId: 1,
  ...
  state = {
    ...
    render() {
      ...
      return (
      <Container>
        <ImageBackground source={require('../../assets/ExplainerImageFive.png')} style={styles.backgroundImageBg}>
          <Row style={{top: 0, position: 'absolute'}}>
            <Button large warning style={styles.largeButton}
            onPress={() => this.props.navigation.navigate('ExplainerImageSix', {
              gameId: gameId,
              displayId: displayId,
              }
            )} >
            <Col style={{alignItems: 'flex-start', marginTop: 40}}>
              <Row>
                <Text style={styles.buttonTextExplain}>Choose <Text style={{backgroundColor: '#ff8300', color: '#fff'}}>Medium</Text> if you want to keep the score rolling-over without the risk of too many wickets</Text>
              </Row>
              <Row>
...
export default connect(mapStateToProps)(ExplainerImageFive);
